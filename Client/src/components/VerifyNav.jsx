...
const VerifyNav = ()=>{
    ...
    const [mobileOpen1, setMobileOpen1] = useState(false);
    const links1=[
            ...
            <span className="text-blue-600">Flow</span>
                ...
                <div className="hidden md:flex items-center gap-1 px-2">
                    {links1.map((link) => (
                        <ScrollLink
                            spy={true}
                            onSetActive={()=>setActiveSection1(link.to)}
                            key={link.to}
                            to={link.to}
                            smooth={true}
                            duration={500}
                ...
                transition-all duration-300 ${activeSection1 === link.to
                ...
                <button onClick={()=>setMobileOpen1(!mobileOpen1)}
                        className="md:hidden p-1">
                    {mobileOpen1 ? <X className="w-5 h-5"/> : <Menu className="w-5 h-5"/>}
                </button>
            </nav>
            <AnimatePresence>
                {mobileOpen1 && (
